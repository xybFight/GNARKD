...
# This file is used to test the performance of GNARKD-POMO for CVRP
...
# test data
problems = torch.load('../../Test_data/CVRP/vrp' + str(args.problem_size) + '_test_seed1234.pt')
args.nb_batch_eval = (10000 + args.batch_size - 1) // args.batch_size
val_depot_xy = problems['depot_xy'].to(model_args.device)
val_node_xy = problems['node_xy'].to(model_args.device)
val_node_demand = problems['node_demand'].to(model_args.device)
f = open('checkpoint\checkpoint-n50.pkl', 'rb') if args.problem_size == 50 else open('checkpoint\checkpoint-n100.pkl',
                                                                                     'rb')
# This is for different distillation temperature
...
with torch.no_grad():
    ...
    for step in range(args.nb_batch_eval):
        ...
        if args.aug_factor != 8:
            ...
            # Beam search
            # selected_node_list = beam_search(score_stu, node_demand)
            ...
            # POMO with 8 augment
            selected_node_list = pomo_beam_search(score_stu, node_demand)
            L_val_aug_pomo = compute_tour_length_all_b(torch.cat((depot_xy, node_xy), dim=1),
                                                       selected_node_list).reshape(8, -1, args.pomo_size)
            # Beam search with 8 augment
            # selected_node_list = beam_search(score_stu, node_demand)
            # L_val_aug_pomo = compute_tour_length_all_b(torch.cat((depot_xy, node_xy), dim=1), selected_node_list).reshape(8, -1, 1000)
