...
# This file is used to test the performance of GNARKD-TM for TSP
...
def greedy_search(attn_weight_all):
    ...
    for node_index in range(nb_nodes - 1):
        attn_weight = torch.softmax(attn_weight_all[zero_to_bsz, idx].masked_fill(mask_visited_nodes, float('-1e10')),
                                    dim=-1)
        # actions_pro_dis.append(attn_weight.squeeze(0).cpu().detach().numpy())
    ...
    # return tour_nar, actions_pro_dis
    return tour_nar
...
x_10k = torch.load('../../Test_data/TSP/10k_TSP50.pt').to(device) if args.nb_nodes == 50 else torch.load('../../Test_data/TSP/10k_TSP100.pt').to(device)
args.bsz = 1000

# following the TM model
args.dim_emb = 128
args.dim_ff = 512
...
student_model = NAR_TSP_net(args.dim_input_nodes, args.dim_emb, args.dim_ff, args.nb_layers_encoder,
                            ...
                            args.nb_heads)
f = open('checkpoint/checkpoint-n50.pkl', 'rb') if args.nb_nodes == 50 else open('checkpoint/checkpoint-n100.pkl', 'rb')
# for different temp
# f = open('Temp_trained/temp_0.5.pkl', 'rb')
...
with torch.no_grad():
    ...
    for step in range(args.nb_batch_eval):
        ...
        elif args.nb_nodes <= B <= args.nb_nodes ** 2:
